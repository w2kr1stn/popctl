...
class TestEffectiveModel:
    ...
    def test_effective_model_default_codex(self) -> None:
        """effective_model returns the Codex default when no model is configured."""
        config = AdvisorConfig(provider="codex", model=None)

        assert config.effective_model == DEFAULT_MODELS["codex"]
        assert config.effective_model == "gpt-5.6-terra"
...
""")
    ...
    def test_load_invalid_api_key_redacts_input_value(
        self,
        tmp_path: Path,
        caplog: pytest.LogCaptureFixture,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Invalid API key values never appear in errors or corruption warnings."""
        canary = "api-key-canary-must-not-leak"
        config_file = tmp_path / "advisor.toml"
        config_file.write_text(f'api_key = ["{canary}"]\n')

        with pytest.raises(AdvisorConfigError, match=r"advisor\.toml is invalid: api_key") as exc:
            load_advisor_config(config_file)

        assert canary not in str(exc.value)

        monkeypatch.setattr("popctl.advisor.config.get_config_dir", lambda: tmp_path)
        load_or_create_config()

        assert canary not in caplog.text
        assert "advisor.toml is invalid: api_key" in caplog.text
...
class TestSaveAdvisorConfig:
    ...
    def test_save_config(self, tmp_path: Path) -> None:
        ...
        assert loaded.api_key == "gemini-key"

    def test_save_config_uses_private_file_mode(self, tmp_path: Path) -> None:
        """New advisor configs are readable and writable only by the owner."""
        config_file = tmp_path / "advisor.toml"

        save_advisor_config(AdvisorConfig(), config_file)

        assert S_IMODE(config_file.stat().st_mode) == 0o600

    def test_save_config_restricts_existing_file_mode(self, tmp_path: Path) -> None:
        """Rewriting a legacy advisor config removes group and world access."""
        config_file = tmp_path / "advisor.toml"
