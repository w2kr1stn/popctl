...
class TestIsProtectedShared:
    ...
    def test_gnome_and_kde_paths_protected(
        self, domain: Literal["filesystem", "configs"], relative_path: str
    ) -> None:
        """GNOME and KDE desktop configuration paths should be protected."""
        assert is_protected(f"{self._home()}/{relative_path}", domain) is True

    @pytest.mark.parametrize("domain", _DOMAIN_PARAMS)
    @pytest.mark.parametrize(
        ("relative_root", "relative_child"),
        [
            (".config/dconf", ".config/dconf/user"),
            (".config/gnome-session", ".config/gnome-session/saved-session"),
        ],
    )
    def test_session_configuration_directories_and_descendants_are_protected(
        self,
        domain: Literal["filesystem", "configs"],
        relative_root: str,
        relative_child: str,
    ) -> None:
        """Session-critical directories and their direct children are protected."""
        home = self._home()
        assert is_protected(f"{home}/{relative_root}", domain) is True
        assert is_protected(f"{home}/{relative_child}", domain) is True
    ...
    def test_unrelated_gnome_and_kde_paths_are_not_protected(
        self, domain: Literal["filesystem", "configs"], relative_path: str
    ) -> None:
        """Non-session GNOME and KDE configuration paths should remain cleanable."""
        assert is_protected(f"{self._home()}/{relative_path}", domain) is False

    @pytest.mark.parametrize("domain", _DOMAIN_PARAMS)
    def test_unrelated_desktop_path_not_protected(
        self, domain: Literal["filesystem", "configs"]
    ) -> None:
        """An unrelated desktop application path should remain unprotected."""
        path = f"{self._home()}/.local/share/unrelated-desktop-app"
        assert is_protected(path, domain) is False
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ".local/share/gnome-shell/extensions/example@extensions.gnome.org",
        ],
    )
    def test_gnome_shell_extensions_are_not_protected(
        self, domain: Literal["filesystem", "configs"], relative_path: str
    ) -> None:
        """Per-user GNOME extensions are optional customisation."""
        assert is_protected(f"{self._home()}/{relative_path}", domain) is False
...
class TestIsProtectedFilesystem:
    ...
    def test_local_state_popctl_root_and_descendants_are_protected(self) -> None:
        """popctl state must remain protected from filesystem removal."""
        home = self._home()
        assert is_protected(f"{home}/.local/state/popctl", "filesystem") is True
        assert (
            is_protected(f"{home}/.local/state/popctl/sessions/20260101T000000", "filesystem")
            is True
        )
        assert is_protected(f"{home}/.local/state/other-app", "filesystem") is False
