...
class TestSessionClaudeMdTemplate:
    ...
    def test_session_claude_md_uses_manifest_aligned_ask_first_philosophy(self) -> None:
        """SESSION_CLAUDE_MD preserves user intent when classification is uncertain."""
        assert "## Design Philosophy" in SESSION_CLAUDE_MD
        assert "Keep the system close to the user's declared manifest." in SESSION_CLAUDE_MD
        assert "observable usage evidence on the system" in SESSION_CLAUDE_MD
        assert "the user's answers" in SESSION_CLAUDE_MD
        assert "prefer ASK over REMOVE" in SESSION_CLAUDE_MD
        assert "gcc, make" not in SESSION_CLAUDE_MD
    ...
    def test_session_claude_md_has_session_close(self) -> None:
        """SESSION_CLAUDE_MD has an English formal session close instruction."""
        assert "**Session complete.**" in SESSION_CLAUDE_MD
        assert "You can now close the session." in SESSION_CLAUDE_MD
