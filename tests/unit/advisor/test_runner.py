...
class TestRunHeadlessHost:
    ...
    def test_success_redacts_api_key_from_captured_output(self, tmp_path: Path) -> None:
        ws = self._make_workspace(tmp_path)
        (ws / "output" / "decisions.toml").write_text("[packages.apt]")
        canary = "host-success-api-key"
        mock_result = MagicMock(
            success=True,
            stdout=f"output {canary} and {canary}",
            stderr=f"stderr {canary}",
            returncode=0,
        )

        with patch("popctl.advisor.runner.run_command", return_value=mock_result):
            result = AgentRunner(AdvisorConfig(api_key=canary)).run_headless(ws)
    ...
    def test_failure_redacts_api_key_from_captured_output(self, tmp_path: Path) -> None:
        ws = self._make_workspace(tmp_path)
        canary = "host-failure-api-key"
        mock_result = MagicMock(
            success=False,
            stdout=f"output {canary}",
            stderr=f"stderr {canary} and {canary}",
            returncode=1,
        )
    ...
    def test_headless_host_passes_api_key(
        self,
        tmp_path: Path,
        provider: str,
        env_var: str,
        ...
        ws = self._make_workspace(tmp_path)
        mock_result = MagicMock(success=False, stdout="", stderr="", returncode=1)
        with patch("popctl.advisor.runner.run_command", return_value=mock_result) as mock_run:
            AgentRunner(AdvisorConfig(provider=provider, api_key="provider-key")).run_headless(ws)
        assert mock_run.call_args.kwargs["env"][env_var] == "provider-key"
...
class TestLaunchInteractiveHost:
    ...
    def test_interactive_host_passes_provider_api_key(
        self,
        tmp_path: Path,
        provider: str,
        env_var: str,
        ...
        with (
            patch("shutil.which", return_value=f"/usr/bin/{provider}"),
            patch("sys.stdin") as mock_stdin,
            patch("popctl.advisor.runner.run_interactive", return_value=0) as mock_run,
        ):
            mock_stdin.isatty.return_value = True
            agent_runner = AgentRunner(AdvisorConfig(provider=provider, api_key="provider-key"))
            agent_runner.launch_interactive(tmp_path)
        assert mock_run.call_args.kwargs["env"][env_var] == "provider-key"
    ...
    def test_manual_instructions_include_provider(self, tmp_path: Path) -> None:
        ...
        assert "Run `popctl doctor` to check your setup" in result.output
        assert "rerun with `--no-advisor` to skip AI classification" in result.output
...
class TestRunHeadlessSession:
    ...
    def test_success_redacts_api_key_from_captured_output(self, tmp_path: Path) -> None:
        ws = self._make_workspace(tmp_path)
        (ws / "output" / "decisions.toml").write_text("[packages.apt]")
        canary = "session-success-api-key"
        session = MagicMock()
        session.run_headless.return_value = _FakeSessionResult(
            returncode=0,
            stdout=f"output {canary} and {canary}",
        ...
        result = AgentRunner(
            AdvisorConfig(api_key=canary), session=session,
        )._run_headless_session(ws)
    ...
    def test_failure_redacts_api_key_from_captured_output(self, tmp_path: Path) -> None:
        ws = self._make_workspace(tmp_path)
        canary = "session-failure-api-key"
        session = MagicMock()
        session.run_headless.return_value = _FakeSessionResult(
            returncode=1,
            stdout=f"output {canary}",
    ...
    def test_passes_configured_provider_to_session(self, tmp_path: Path) -> None:
        ws = self._make_workspace(tmp_path)
        (ws / "output" / "decisions.toml").write_text("[packages.apt]")
        session = MagicMock()
        session.run_headless.return_value = _FakeSessionResult(
        ...
        AgentRunner(AdvisorConfig(provider="gemini"), session=session).run_headless(ws)

        assert session.run_headless.call_args.kwargs["agent"] == "gemini"

    def test_passes_codex_effective_model_to_session(self, tmp_path: Path) -> None:
        ws = self._make_workspace(tmp_path)
        (ws / "output" / "decisions.toml").write_text("[packages.apt]")
        session = MagicMock()
        session.run_headless.return_value = _FakeSessionResult(
        ...
        AgentRunner(AdvisorConfig(provider="codex"), session=session).run_headless(ws)

        call_kwargs = session.run_headless.call_args.kwargs
        assert call_kwargs["agent"] == "codex"
        assert call_kwargs["model"] == "gpt-5.6-terra"

    def test_passes_gemini_effective_model_to_session(self, tmp_path: Path) -> None:
        ws = self._make_workspace(tmp_path)
        (ws / "output" / "decisions.toml").write_text("[packages.apt]")
        session = MagicMock()
        session.run_headless.return_value = _FakeSessionResult(
    ...
    def test_api_key_warns_and_falls_back_to_host(self, tmp_path: Path) -> None:
        session = MagicMock()
        host_result = AgentResult(success=False, output="", error="host execution")
        runner = AgentRunner(AdvisorConfig(api_key="provider-key"), session=session)

        with (
            patch.object(runner, "_run_headless_host", return_value=host_result) as mock_host,
...
class TestLaunchInteractiveSession:
    ...
    def test_success_with_decisions(self, tmp_path: Path) -> None:
        ...
        runner = AgentRunner(AdvisorConfig(provider="gemini"), session=session)
        result = runner.launch_interactive(ws)
        assert result.success is True
        assert result.decisions_path == ws / "output" / "decisions.toml"
        assert session.run_interactive.call_args.kwargs["agent"] == "gemini"
    ...
    def test_api_key_warns_and_falls_back_to_host(self, tmp_path: Path) -> None:
        ...
        with (
            patch.object(runner, "_launch_host", return_value=host_result) as mock_host,
            patch("popctl.advisor.runner.print_warning") as mock_warn,
        ):
...
class TestBuildCommands:
    ...
    def test_build_headless_codex(self) -> None:
        cmd = AgentRunner(AdvisorConfig(provider="codex"))._build_headless_command()
        assert cmd[:2] == ["codex", "exec"]
        assert cmd[2:5] == ["--skip-git-repo-check", "--model", "gpt-5.6-terra"]
        assert cmd[-1] == INITIAL_PROMPT

    def test_build_interactive_codex(self) -> None:
        cmd = AgentRunner(AdvisorConfig(provider="codex"))._build_interactive_command()
        assert cmd[0] == "codex"
        # The interactive TUI rejects --skip-git-repo-check (exec-only flag).
        assert "--skip-git-repo-check" not in cmd
        assert cmd[1:3] == ["--model", "gpt-5.6-terra"]
        assert cmd[-1] == INITIAL_PROMPT
    ...
    def test_codex_headless_closes_stdin(self, tmp_path: Path) -> None:
        workspace = tmp_path / "workspace"
        workspace.mkdir()
        (workspace / "output").mkdir()
        mock_result = MagicMock(stdout="", stderr="", returncode=1)
        with patch("popctl.advisor.runner.subprocess.run", return_value=mock_result) as mock_run:
            agent_runner = AgentRunner(AdvisorConfig(provider="codex", api_key="codex-key"))
            agent_runner.run_headless(workspace)
