from __future__ import annotations

import logging
import os
import shutil
import subprocess
import sys
from dataclasses import dataclass, field
from pathlib import Path

from popctl.advisor.config import AdvisorConfig, AdvisorProvider
from popctl.advisor.prompts import INITIAL_PROMPT
from popctl.advisor.session_protocol import DjinnSessionManager
from popctl.core.paths import ensure_dir, get_state_dir
from popctl.utils.formatting import print_warning
from popctl.utils.shell import CommandResult, run_command, run_interactive

MANUAL_MODE_SENTINEL: str = "manual_mode"

# Ensure rich color output in advisor sessions.
_SESSION_ENV: dict[str, str] = {
    "TERM": "xterm-256color",
    "COLORTERM": "truecolor",
}

_API_KEY_ENV_VARS: dict[AdvisorProvider, str] = {
    "claude": "ANTHROPIC_API_KEY",
    "gemini": "GEMINI_API_KEY",
    "codex": "OPENAI_API_KEY",
}


@dataclass(frozen=True, slots=True)
class AgentResult:

    success: bool
    output: str
    error: str | None = None
    decisions_path: Path | None = None
    workspace_path: Path | None = None


@dataclass
class AgentRunner:

    config: AdvisorConfig
    session: DjinnSessionManager | None = field(default=None)

    # ── Public API ──────────────────────────────────────────────

    def run_headless(self, workspace_dir: Path) -> AgentResult:
        if self.session is not None and not self.config.api_key:
            return self._run_headless_session(workspace_dir)
        if self.session is not None:
            self._warn_session_api_key_fallback()
        return self._run_headless_host(workspace_dir)

    def launch_interactive(self, workspace_dir: Path) -> AgentResult:
        if self.session is not None and not self.config.api_key:
            return self._launch_interactive_session(workspace_dir)
        if self.session is not None:
            self._warn_session_api_key_fallback()
        return self._launch_host(workspace_dir)

    # ── SessionManager mode ──────────────────────────────────────

    def _run_headless_session(self, workspace_dir: Path) -> AgentResult:
        assert self.session is not None
        try:
            result = self.session.run_headless(
                workspace_dir=workspace_dir,
                prompt=INITIAL_PROMPT,
                timeout=self.config.timeout_seconds,
                **self._session_agent_kwargs(),
            )
        except ValueError as e:
            return AgentResult(
                success=False, output="", error=str(e), workspace_path=workspace_dir,
            )

        output = self._redact_captured_output(result.stdout)
        error = self._redact_captured_output(result.stderr)
        self._persist_memory(workspace_dir / "memory.md")
        decisions_path = workspace_dir / "output" / "decisions.toml"
        if result.success and decisions_path.exists():
            return AgentResult(
                success=True,
                output=output,
                decisions_path=decisions_path,
                workspace_path=workspace_dir,
            )
        return AgentResult(
            success=False,
            output=output,
            error=error or f"Agent exited with code {result.returncode}",
            workspace_path=workspace_dir,
        )

    def _launch_interactive_session(self, workspace_dir: Path) -> AgentResult:
        assert self.session is not None
        try:
            result = self.session.run_interactive(
                workspace_dir=workspace_dir,
                initial_prompt=INITIAL_PROMPT,
                **self._session_agent_kwargs(),
            )
        except ValueError as e:
            return AgentResult(
                success=False, output="", error=str(e), workspace_path=workspace_dir,
            )

        output = self._redact_captured_output(result.stdout)
        error = self._redact_captured_output(result.stderr)
        if result.returncode != 0:
            return AgentResult(
                success=False,
                output=output,
                error=error or f"Session exited with code {result.returncode}",
                workspace_path=workspace_dir,
            )
        return self._post_session_result(workspace_dir)

    # ── Host mode ───────────────────────────────────────────────

    def _launch_host(self, workspace_dir: Path) -> AgentResult:
        if not shutil.which(self.config.provider):
            return self._manual_instructions(workspace_dir)
        if not sys.stdin.isatty():
            return self._run_headless_host(workspace_dir)
        return self._exec_host_interactive(workspace_dir)

    def _exec_host_interactive(self, workspace_dir: Path) -> AgentResult:
        cmd = self._build_interactive_command()
        # The provider inherits the user's terminal; an echoed key stays there.
        # popctl captures and persists none of the interactive output.
        run_interactive(cmd, cwd=str(workspace_dir), env=self._host_env())
        return self._post_session_result(workspace_dir)

    def _run_headless_host(self, workspace_dir: Path) -> AgentResult:
        command = self._build_headless_command()

        try:
            result = self._run_headless_command(command, workspace_dir)
        except subprocess.TimeoutExpired:
            return AgentResult(
                success=False,
                output="",
                error=f"Agent execution timed out after {self.config.timeout_seconds} seconds",
                workspace_path=workspace_dir,
            )
        except (FileNotFoundError, OSError) as e:
            return AgentResult(
                success=False,
                output="",
                error=f"Agent command failed: {e}",
                workspace_path=workspace_dir,
            )

        output = self._redact_captured_output(result.stdout)
        error = self._redact_captured_output(result.stderr)
        decisions_path = workspace_dir / "output" / "decisions.toml"
        if result.success and decisions_path.exists():
            return AgentResult(
                success=True,
                output=output,
                decisions_path=decisions_path,
                workspace_path=workspace_dir,
            )
        return AgentResult(
            success=False,
            output=output,
            error=error or f"Agent exited with code {result.returncode}",
            workspace_path=workspace_dir,
        )

    # ── Shared helpers ──────────────────────────────────────────

    def _session_agent_kwargs(self) -> dict[str, str]:
        return {"agent": self.config.provider, "model": self.config.effective_model}

    @staticmethod
    def _warn_session_api_key_fallback() -> None:
        print_warning(
            "djinn session backend does not support api_key environment forwarding; "
            "falling back to host execution",
        )

    def _build_interactive_command(self) -> list[str]:
        provider = self.config.provider
        if provider == "claude":
            return ["claude", INITIAL_PROMPT, *self._model_flags()]
        if provider == "gemini":
            return ["gemini", "--prompt", INITIAL_PROMPT, *self._model_flags()]
        # The interactive codex TUI rejects --skip-git-repo-check (exec-only flag)
        # and prompts the user itself when the workspace is not a git repository.
        return ["codex", *self._model_flags(), INITIAL_PROMPT]

    def _build_headless_command(self) -> list[str]:
        provider = self.config.provider
        if provider == "claude":
            return ["claude", "-p", INITIAL_PROMPT, "--output-format", "json", *self._model_flags()]
        if provider == "gemini":
            return ["gemini", "--prompt", INITIAL_PROMPT, *self._model_flags()]
        return ["codex", "exec", "--skip-git-repo-check", *self._model_flags(), INITIAL_PROMPT]

    def _model_flags(self) -> list[str]:
        if self.config.model or self.config.provider == "codex":
            return ["--model", self.config.effective_model]
        return []

    def _host_env(self) -> dict[str, str]:
        if not self.config.api_key:
            return _SESSION_ENV
        return {
            **_SESSION_ENV,
            _API_KEY_ENV_VARS[self.config.provider]: self.config.api_key,
        }

    def _redact_captured_output(self, output: str) -> str:
        if not self.config.api_key:
            return output
        return output.replace(self.config.api_key, "********")

    def _run_headless_command(
        self,
        command: list[str],
        workspace_dir: Path,
    ) -> CommandResult:
        if self.config.provider != "codex":
            return run_command(
                command,
                timeout=float(self.config.timeout_seconds),
                cwd=str(workspace_dir),
                env=self._host_env(),
            )

        result = subprocess.run(
            command,
            capture_output=True,
            text=True,
            check=False,
            timeout=float(self.config.timeout_seconds),
            cwd=str(workspace_dir),
            env={**os.environ, **self._host_env()},
            stdin=subprocess.DEVNULL,
        )
        return CommandResult(
            stdout=result.stdout,
            stderr=result.stderr,
            returncode=result.returncode,
        )

    @staticmethod
    def manual_mode_hint(provider: AdvisorProvider) -> str:
        return (
            f"The configured {provider} CLI is unavailable. Run `popctl doctor` to check your "
            "setup, or rerun with `--no-advisor` to skip AI classification."
        )

    def _manual_instructions(self, workspace_dir: Path) -> AgentResult:
        provider = self.config.provider
        return AgentResult(
            success=False,
            output=(
                f"{self.manual_mode_hint(provider)}\n"
                f"\n"
                f"Detailed manual instructions:\n"
                f"Workspace prepared: {workspace_dir}\n"
                f"\n"
                f"To start manually:\n"
                f"  cd {workspace_dir}\n"
                f'  {provider} "{INITIAL_PROMPT}"\n'
                f"\n"
                f"After classification:\n"
                f"  popctl advisor apply\n"
            ),
            error=MANUAL_MODE_SENTINEL,
            workspace_path=workspace_dir,
        )

    def _post_session_result(self, workspace_dir: Path) -> AgentResult:
        self._persist_memory(workspace_dir / "memory.md")
        return self._decisions_result(workspace_dir)

    @staticmethod
    def _decisions_result(workspace_dir: Path) -> AgentResult:
        decisions = workspace_dir / "output" / "decisions.toml"
        return AgentResult(
            success=decisions.exists(),
            output="",
            decisions_path=decisions if decisions.exists() else None,
            workspace_path=workspace_dir,
        )

    def _persist_memory(self, workspace_memory: Path) -> None:
        if not workspace_memory.exists():
            return
        logger = logging.getLogger(__name__)
        try:
            advisor_dir = ensure_dir(get_state_dir() / "advisor", "advisor memory")
            persistent_path = advisor_dir / "memory.md"
            shutil.copy2(workspace_memory, persistent_path)
            logger.debug("Persisted memory.md to %s", persistent_path)
        except (OSError, RuntimeError) as e:
            logger.warning("Could not persist memory.md: %s", e)
            print_warning(f"Could not persist advisor memory: {e}")
