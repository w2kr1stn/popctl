...
def _apt_run_command_side_effect() -> Callable[..., CommandResult]:
    """Return successful APT results, including parseable removal simulations."""

    def side_effect(command: list[str], **_kwargs: object) -> CommandResult:
        if "apt-get" in command and {"-s", "--simulate"} & set(command):
            packages = command[command.index("--") + 1 :]
...
class TestSyncDryRun:
    ...
    def test_sync_dry_run_shows_diff(
        ...
        with (
            ...
            result = runner.invoke(app, ["sync", "--dry-run", "--no-filesystem", "--no-configs"])
...
def test_sync_no_advisor_skips_classification(
    ...
    with (
        ...
        mock_run.side_effect = _apt_run_command_side_effect()

        result = runner.invoke(
            app, ["sync", "--no-advisor", "--yes", "--no-filesystem", "--no-configs"]
        )
...
class TestSyncAdvisor:
    ...
    def test_sync_review_runs_advisor_when_in_sync(
        ...
        with (
            ...
            result = runner.invoke(
                app, ["sync", "--review", "--no-advisor", "--no-filesystem", "--no-configs"]
            )
    ...
    def test_sync_review_invokes_advisor_without_new(
        ...
        with (
            ...
            result = runner.invoke(
                app, ["sync", "--review", "--auto", "--no-filesystem", "--no-configs"]
            )
    ...
    def test_sync_advisor_failure_continues(
        ...
        with (
            ...
            # Patch the name where sync.py binds it, not the source module —
            # otherwise the real advisor runs and execs a real agent CLI.
            patch(
                "popctl.cli.commands.sync.load_or_create_config",
            ...
            result = runner.invoke(
                app, ["sync", "--yes", "--auto", "--no-filesystem", "--no-configs"]
            )
...
def test_sync_purge_uses_purge_command(sample_manifest: Manifest) -> None:
    ...
    with (
        ...
        result = runner.invoke(
            app, ["sync", "--yes", "--purge", "--no-filesystem", "--no-configs"]
        )

    # Should have called apt-get purge after the safety simulation.
    assert result.exit_code == 0
    args = next(
        call.args[0]
        for call in mock_run.call_args_list
        if "purge" in call.args[0] and "-s" not in call.args[0] and "--simulate" not in call.args[0]
    )
...
class TestSyncFilesystem:
    ...
    def test_sync_filesystem_phases_run_when_packages_in_sync(
        ...
        with (
            ...
            patch(
                "popctl.cli.commands.sync._invoke_advisor", return_value=None
            ) as mock_invoke_advisor,
        ):
            result = runner.invoke(app, ["sync", "--no-configs"])

        assert result.exit_code == 0
        assert "1 orphaned filesystem" in result.stdout
        mock_invoke_advisor.assert_called_once()
        assert mock_invoke_advisor.call_args.kwargs["domain"] == "filesystem"
...
class TestSyncConfigs:
    ...
    def test_sync_includes_config_phases(
        ...
        mock_invoke_advisor.assert_called_once()
        assert mock_invoke_advisor.call_args.kwargs["domain"] == "configs"
...
class TestInvokeAdvisor:
    ...
    def test_invoke_advisor_selects_sessions_dir_for_session_backend(
        self,
        tmp_path: Path,
        session: object | None,
        ...
        with (
            patch("popctl.cli.commands.sync.load_or_create_config", return_value=MagicMock()),
            patch("popctl.cli.commands.sync.scan_system", return_value=MagicMock()),
            patch(
                "popctl.cli.commands.sync.get_session_manager",
            ...
            patch(
                "popctl.cli.commands.sync.get_manifest_path",
                return_value=tmp_path / "manifest.toml",
            ),
            patch("popctl.cli.commands.sync.get_state_dir", return_value=tmp_path),
            patch(
    ...
    def test_invoke_advisor_scan_failure_returns_none(self) -> None:
        ...
        assert result is None
        mock_scan_system.assert_called_once()
    ...
    def test_invoke_advisor_runner_failure_returns_none(self) -> None:
        ...
        with (
            patch("popctl.cli.commands.sync.load_or_create_config", return_value=mock_config),
    ...
    def test_invoke_advisor_manual_mode_returns_none(self) -> None:
        """Manual mode prints a concise setup hint and returns None."""
        ...
        mock_agent_result = AgentResult(
            success=False,
            output="Detailed manual instructions:\n  cd /tmp/ws\n  gemini \"prompt\"",
            error="manual_mode",
        )
        ...
        with (
            patch(
                "popctl.cli.commands.sync.load_or_create_config",
                return_value=AdvisorConfig(provider="gemini"),
            ),
            patch("popctl.cli.commands.sync.scan_system", return_value=MagicMock()),
            patch(
                "popctl.cli.commands.sync.create_session_workspace",
            ...
            ) as mock_launch_interactive,
            patch("popctl.cli.commands.sync.print_warning") as mock_warning,
            patch("popctl.cli.commands.sync.console.print") as mock_console_print,
        ):
            result = _invoke_advisor(auto=False, domain="packages")

        assert result is None
        mock_launch_interactive.assert_called_once()
        mock_warning.assert_called_once_with(
            "The configured gemini CLI is unavailable. Run `popctl doctor` to check your "
    ...
    def test_invoke_advisor_import_failure_returns_none(self, tmp_path: Path) -> None:
        ...
        assert result is None
        mock_import.assert_called_once_with(decisions_path)
